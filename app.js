(function () {
  ...
  function loadState() {
    try {
      ...
      return {
        events: Array.isArray(stored.events) ? stored.events.map(normalizeStoredEvent) : [],
  ...
  function normalizeStoredEvent(event) {
    ...
    if (!normalized.attendanceStatus) {
      normalized.attendanceStatus = normalized.attendedAt ? "attended" : "pending";
    }
    if (normalized.attendanceStatus !== "attended") {
      normalized.attendedAt = null;
    }
    return normalized;
  }
  ...
  function createEventCard(event) {
    ...
    const attendanceStatus = event.attendanceStatus || (event.attendedAt ? "attended" : "pending");
    article.querySelector(".attended-badge").hidden = attendanceStatus !== "attended";
    article.querySelector(".not-attended-badge").hidden = attendanceStatus !== "not-attended";
    article.querySelector('[data-field="cardBlock"]').hidden = !event.hasCardImage;
    article.querySelectorAll('[data-action="attendance"]').forEach(function (input) {
      input.name = "attendance-" + event.id;
      input.checked = input.value === attendanceStatus;
    });
  ...
  function handleAttendanceChange(event) {
    if (!event.target.matches('input[data-action="attendance"]')) {
      return;
    }
    ...
    savedEvent.attendanceStatus = event.target.value;
    savedEvent.attendedAt = event.target.value === "attended" ? new Date().toISOString() : null;
    persist();
    render();
    if (event.target.value === "attended") {
      showToast("Marked as attended.");
    } else if (event.target.value === "not-attended") {
      showToast("Marked as not attended.");
    } else {
      showToast("Attendance status reset to pending.");
    }
  }
